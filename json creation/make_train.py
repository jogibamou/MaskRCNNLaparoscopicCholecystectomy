...
for key in keys:
    img = cv2.imread('train/' + key.split('.jpg')[0] + '.jpg')
    height = img.shape[0]
    width = img.shape[1]
    current_id = id_count
    id_count += 1
    file_name = 'train/' + key.split('.jpg')[0]
    license = 0
    flickr_url = "yes"
    coco_url = "no"
    date_captured = datetime.datetime.now()
    new_data['images'].append({"id": id_count, "width": width, "height": height, 
...
for key in keys:
    ...
    file_name = 'train/' + key.split('.jpg')[0]+ '.jpg'
    license = 0
    flickr_url = "yes"
    coco_url = "no"
...
with open('data_train_.json', 'w') as out:
